...
In this section we will cover how a hosted cluster can be destroyed from the WebUI and from the CLI.

[#destroying-hostedcluster-webui]
== Destroying the Hosted Cluster from the WebUI
...
[#cleaning-hostedcluster-namespace]
== Cleaning up hosted cluster namespace

When we deploy a hosted cluster, there are two namespaces that are created. One namespaces is where the control plane pods run and the other one is where the HostedCluster and NodePool resources are created. This second namespaces won't be removed automatically, we need to do it manually.

1. Delete the hosted namespace
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig delete namespace hosted
-----
+
[console-input]
[source,console]
-----
namespace "hosted" deleted
-----