...
In this section, we install the Metallb operator and configure it so that our installation is fully accessible through a `LoadBalancer` IP  that will point to our ingress service.
...
We can use the following loop to wait for the CRD to be available
...
With this available load balancer IP, we will patch the ingress service on our deployed cluster so that it uses it
