...
(with so many yaml I would have liked to just download all in a folder at the beginning)
  (why not from the official documentation https://docs.openshift.com/container-platform/4.13/networking/metallb/metallb-operator-install.html)
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
(how in a net diagram you access the cluster? is the api traffic goes to management cluster? probably yes.
 Maybe worth point out the ".apps." part.
