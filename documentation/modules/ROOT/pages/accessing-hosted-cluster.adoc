...
1. Go to the `hosted` cluster view if you're not already on it. You can get back to it by opening the https://console-openshift-console.apps.management.hypershift.lab/[OpenShift Console] (or by using the pre-configured Firefox image:bookmark-hub-ocp-console.png[Management OCP Console bookmark icon] bookmark). Making sure `All Clusters` is selected and clicking on the cluster named `hosted`. 
