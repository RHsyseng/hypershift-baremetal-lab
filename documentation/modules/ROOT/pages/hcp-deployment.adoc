...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig -n multicluster-engine wait --for=jsonpath='{.status.state}'=AtLatestKnown subscription/multicluster-engine --timeout=300s
-----
+
[console-input]
[source,console]
