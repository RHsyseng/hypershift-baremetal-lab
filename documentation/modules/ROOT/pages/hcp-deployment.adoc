...
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig login -u admin \
    -p {hub_admin_password} https://api.management.hypershift.lab:6443 \
...
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig label clusterimageset \
    img{hosted-cluster-version-1}-x86-64-appsub visible=true --overwrite
-----

[console-input]
[source,console,subs="attributes+,+macros"]
-----
clusterimageset.hive.openshift.io/img{hosted-cluster-version-1}-x86-64-appsub labeled
-----
