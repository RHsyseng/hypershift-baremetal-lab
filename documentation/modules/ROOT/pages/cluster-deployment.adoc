...
In this section, we will deploy the assets needed to create an OpenShift cluster and launch baremetal-like VMs that will be used up as the worker nodes of our cluster

[#create-hosted-cluster]
== Create Hosted Cluster

We will render a YAML such as the following where:
...
* A wildcard DNS entry for `app.$CLUSTER.$DOMAIN` with need to be created for the `CLUSTER` and `DOMAIN` specified
* `RELEASE_IMAGE` comes from `openshift-install` binary, to match the desired target version.
* `MACHINE_CIDR` corresponds to the CIDR of the nodes we will be adding as workers.

NOTE: We will cover how to configure a VIP through metallb to reach the ingress services of the hosted cluster
...
After waiting a bit, the control plane pods show up
...
The nodes will boot via redfish and by means of the infraenv label do so using the ISO associated with the environment.

After a few minutes, they will appear as available agents in the `clusters-$CLUSTER` namespace
...
Once the agents are available, we can create a `nodepool`` to deploy those nodes as workers.

Note that in this case, we will use the same target version for this group of workers, although a different version can be specified.
...
When the number of replicas in the nodepool object is changed, the `capi-provider` component tries to locate an available agent and deploy them as additional workers on the corresponding cluster.

After a while, the nodes will be part of our cluster and be ready for workloads.

We can monitor the progress with the following command
