...
In this section, we install Hypershift. For this, we go through different steps:

* Installing the multi cluster engine operator
* Creating a CR for it with the hypershift addon.
* Configure assisted service, as requisite in order to use the agent provider.
...
We also define a provisioning CR to make sure metal3 operator is running
