...
We create the following YAML to deploy the operator
...
We then define the following CR to deploy a multicluster engine instance with the HyperShift add-on
...
We define a provisioning CR to make sure that the baremetal operator is running
...
Finally, we create the proper configuration for the assisted service, by rendering the following YAML, using `envsubst`
...
Save this file as `scripts/MCE/assisted-service.sample.yaml`

Then launch the following script to render and apply this YAML
...
We can check that the pods are deployed by using
...
The HyperShift operator was also deployed in its own namespace
