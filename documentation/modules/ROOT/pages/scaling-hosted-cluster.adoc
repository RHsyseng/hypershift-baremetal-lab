...
The scaling operations can be done from the WebUI or from the CLI. This section will only cover the CLI method as it's more convenient than the WebUI.
...
-----
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig -n hosted scale nodepool nodepool-hosted-1 --replicas 3
-----
+
[console-input]
[source,console]
...
-----
+
IMPORTANT: You can see the `CLUSTER` is set to `hosted` for the second agent. It can take a few minutes for the agent to be assigned to the Hosted Cluster after scaling the NodePool.
+
[console-input]
[source,console]
-----
...
aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0202   hosted    true       auto-assign   
...
NAME                                   CLUSTER   APPROVED   ROLE     STAGE
aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0201   hosted    true       worker   Done
aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0202   hosted    true       worker   Writing image to disk
aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0203   hosted    true       worker   Done
...
hosted-worker0   Ready    worker   169m    v1.25.10+3fe2906
hosted-worker1   Ready    worker   5m55s   v1.25.10+3fe2906
hosted-worker2   Ready    worker   169m    v1.25.10+3fe2906
-----
+
...
-----
NAME             STATUS                     ROLES    AGE     VERSION
hosted-worker0   Ready                      worker   3h13m   v1.25.10+3fe2906
hosted-worker1   Ready                      worker   29m     v1.25.10+3fe2906
hosted-worker2   Ready,SchedulingDisabled   worker   3h13m   v1.25.10+3fe2906
-----
+
...
-----
+
IMPORTANT: It can take a while for the node to exit the cluster. It will very much depend on the workloads running on it. During our tests it took like 10 minutes.
+
[console-input]
[source,console]
-----
...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig -n hosted patch nodepool nodepool-hosted-1 --type=json -p '[{"op": "remove", "path": "/spec/replicas"},{"op":"add", "path": "/spec/autoScaling", "value": { "max": 3, "min": 2 }}]'
-----
+
[console-input]
[source,console]
...
[source,bash,subs="attributes+,+macros"]
-----
oc --insecure-skip-tls-verify=true --kubeconfig ~/hypershift-lab/hosted-kubeconfig -n default create deployment test-app --image=quay.io/mavazque/reversewords:latest
oc --insecure-skip-tls-verify=true --kubeconfig ~/hypershift-lab/hosted-kubeconfig -n default patch deployment test-app -p '{"spec":{"template":{"spec":{"$setElementOrder/containers":[{"name":"reversewords"}],"containers":[{"name":"reversewords","resources":{"requests":{"cpu":10}}}]}}}}'
...
-----
+
3. If we check the pods we will see that one of the pods is in `Pending` state, that's because the current cluster with two workers cannot schedule the pod due to insufficient vCPU capacity.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
test-app-678487f478-7d8bx   0/1     Pending   0          3s
test-app-678487f478-bgcpj   1/1     Running   0          3s
test-app-678487f478-mwwq9   1/1     Running   0          31s
-----
+
4. At this point the `NodePool` will be scaled automatically, if we check the `NodePool` this is what we see.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
hosted      nodepool-hosted-1   hosted                    2               True          False        4.12.20                                      Scaling up MachineSet to 3 replicas (actual 2)
-----
+
5. The spare agent is joining the cluster.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
hardware-inventory   aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0201   hosted    true       worker   Rebooting
hardware-inventory   aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0202   hosted    true       worker   Done
hardware-inventory   aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0203   hosted    true       worker   Done
-----
+
6. Once the new node joins the cluster the workload will be running.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
NAME             STATUS   ROLES    AGE     VERSION
hosted-worker0   Ready    worker   61m     v1.25.10+3fe2906
hosted-worker1   Ready    worker   61m     v1.25.10+3fe2906
hosted-worker2   Ready    worker   3m15s   v1.25.10+3fe2906
-----
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
...
-----
NAME                        READY   STATUS    RESTARTS   AGE
test-app-678487f478-7d8bx   1/1     Running   0          8m33s
test-app-678487f478-bgcpj   1/1     Running   0          8m33s
test-app-678487f478-mwwq9   1/1     Running   0          9m1s
-----
+
7. If we delete the workload, after 10 minutes the `NodePool` will be automatically scaled down again.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --insecure-skip-tls-verify=true --kubeconfig ~/hypershift-lab/hosted-kubeconfig -n default delete deployment test-app
-----
+
[console-input]
[source,console]
...
-----
+
8. Once the `NodePool` gets scaled down, the hosted cluster will be back to two nodes. 
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --insecure-skip-tls-verify=true --kubeconfig /home/mario/hypershift-lab/hosted-kubeconfig -n default get nodes
-----
+
...
-----
NAME             STATUS                     ROLES    AGE   VERSION
hosted-worker1   Ready                      worker   76m   v1.25.10+3fe2906
hosted-worker2   Ready                      worker   19m   v1.25.10+3fe2906
-----
