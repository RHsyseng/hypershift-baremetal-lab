...
-----
+
3. After a few moments, the installation should begin.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
IMPORTANT: It can take up to five minutes for the installation to start.
+
[console-input]
[source,console]
-----
...
-----
+
IMPORTANT: It can take up to ten minutes for the installation to finish.
+
[console-input]
[source,console]
-----
...
-----
+
6. Now that we have seen how to add a node, let's see how to scale down the Hosted Cluster, thus removing a node. We can run the scale command again requesting 2 replicas.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
IMPORTANT: It can take a while for the node to exit the cluster. It will very much depend on the workloads running on it. During our tests it took around five minutes.
+
[console-input]
[source,console,subs="attributes+,+macros"]
-----
...
-----
+
9. The agent will be unassociated from any cluster and role so it can be reused later.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
1. We need to enable auto-scaling for our `NodePool`, we're setting a minimum of two nodes and a maximum of three. This means that the hosted cluster will add one extra worker when cluster capacity is reached.
+
IMPORTANT: The hosted cluster will be scaled down if the additional capacity has not been used for the past ten minutes.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
2. At this point we need to generate some load. We have two workers with 12 vCPUs, that means that if we have workloads running on the cluster requesting more than 24 vCPUs, extra capacity should be added to the cluster. Let's create such workload.
+
IMPORTANT: We begin with one, but will be scaling up to three replicas, each requesting ten vCPUs.  After we scale the deployment, we will need an extra node to accommodate the three replicas.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
3. If we check the pods we will see that we have one pod in the `Running` state. This is because we are requesting a single replica and have enough capacity in the hosted cluster to run the workload.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
5. If we check the pods we will see that one of the pods is in `Pending` state. This is because the current cluster with two workers cannot schedule the third pod due to insufficient vCPU capacity.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
IMPORTANT: It can take up to ten minutes for the new node to join the cluster.
+
[console-input]
[source,console,subs="attributes+,+macros"]
-----
...
-----
+
9. If we delete the workload, after ten minutes the `NodePool` will be automatically scaled down again.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
IMPORTANT: It can take up to five minutes for the node to leave the cluster.
+
[console-input]
[source,console,subs="attributes+,+macros"]
-----
