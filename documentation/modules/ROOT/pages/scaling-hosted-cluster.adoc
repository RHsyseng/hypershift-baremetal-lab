...
Under [certain conditions](https://issues.redhat.com/browse/MGMT-15024), the node leaving the cluster may end up in an inconsistent state that will prevent its deletion. With below steps we will fix the issue.
...
-----
4. Remove the finalizer from the `AgentMachine`.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/hypershift-lab/mgmt-kubeconfig -n hosted-hosted patch agentmachine nodepool-hosted-1-q8kfx -p '{"metadata":{"finalizers":null}}' --type merge
-----
+
[console-input]
[source,console]
