...
Assisted Service allows us to boot our hardware with a discovery ISO that will inspect our hardware and add it to our hardware inventory. Later on, when we create our first Hosted Cluster the hardware from this inventory will be used to provision the required data plane nodes.
...
-----
+
2. We need a valid pull secret in our hardware inventory namespace. Run the following command to copy the management cluster's pull secret there. 
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
4. The `InfraEnv` should have generated an ISO, let's check it.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
6. After a few moments the `BareMetalHosts` should move to the `Provisioning` state.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
-----
+
IMPORTANT: It can take up to two minutes for the BMHs to move to provisioned state. Sometimes you may see one of the BMHs go into `Provisioning error`, this error should be recovered automatically.
+
[console-input]
[source,console]
-----
...
-----
+
7. At this point the bare metal nodes are being booted with the discovery ISO and we should see some `Agents` created. 
+
