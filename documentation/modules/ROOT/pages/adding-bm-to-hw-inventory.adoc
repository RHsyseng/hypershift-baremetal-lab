...
-----
+
6. After a few moments then `BareMetalHosts` must have moved to `Provisioning` state.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
+
IMPORTANT: It can take up to 10 minutes for the nodes to boot and show up as agents. Keep running the command from time to time.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
