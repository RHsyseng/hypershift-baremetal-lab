...
As you already known, when using Hosted Control Planes the control plane components will run as pods on the management cluster while the data plane will run on dedicated nodes on specific infrastructures. In this lab, we are using bare metal nodes as backing infrastructure for our data plane.
...
-----
5. At this point we are ready to boot our bare metal nodes with this ISO, in order to so we need to createa few `BareMetalHost` objects.
+
IMPORTANT: Below command will create some `Secrets` with credentials for our BMCs and the different `BareMetalHosts`. You can see how we are referencing our `InfraEnv` in the `BareMetalHosts` by using a custom label `infraenvs.agent-install.openshift.io: hosted`, that will make sure our nodes are booted with the ISO generated.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
