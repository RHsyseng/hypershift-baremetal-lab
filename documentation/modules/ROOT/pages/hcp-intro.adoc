...
A Hosted Control Plane Cluster is a form factor of Red Hat OpenShift Container platform, but it follows a distinct architectural model.

In standalone OpenShift, the control plane and data plane are coupled in terms of locality. The network stack is shared and a dedicated group of nodes hosts the control plane with a minimum number to ensure a quorum. While functional, this approach may not always meet customers' diverse use cases, especially when it comes to multi-cluster scale deployments.
...
This alternative approach to run clusters at scale adds new terminology:

* `Management Cluster`: OpenShift cluster running the `Hosted Control Planes` addon and running `virtual` control planes as workloads.
* `Hosted Cluster`: OpenShift cluster that has been deployed via `Hosted Control Planes` and using the virtual control plane components of the management cluster.

Hosted Control Planes for Red Hat OpenShift decouple the control plane from the data plane by:

* Provideing network domain separation between control-plane and workloads.
* Offering a shared interface for fleet administrators and Site Reliability Engineers (SREs) to operate multiple clusters easily.
* Treating the control plane like any other workload, enabling administrators to use the same stack to monitor, secure, and operate their applications while managing the control plane.
...
The decoupling of the control plane and data plane introduces multiple potential benefits and paves the way for a Hybrid-cloud approach. Below are possibilities that Hosted Control Planes as a technology enables.
...
This is the list of providers with their supportability level as OpenShift Hosted Control Planes {hosted-control-planes-version}:

* **Agent (GA)**: Uses Assisted Installer to get the worker nodes provisioned on bare-metal.
* **AWS (GA)**: Deploys workers using the AWS APIs.
* **OpenShift Virtualization (GA)**: Uses OpenShift Virtualization to get the worker nodes provisioned as VMs on the management cluster.
* **None (Tech Preview)**: Provides a ignition config you need to inject into a RHCOS ISO to provision the worker nodes.
* **PowerVS (GA)**: Deploys workers using the IBM Cloud APIs.
